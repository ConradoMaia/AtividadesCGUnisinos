/* Atividade Vivencial 2 - Computação Gráfica
 * Técnica de Iluminação de 3 pontos
 * Adaptado por Conrado Maia Heckler
 * Baseado em src/m4/codigom4.cpp (última atualização em 11/06/2025)
 */
...
#include "Shader.h"
#include "Camera.h" // Include Camera class for camera position

// Global variables for material properties
glm::vec3 Ka_material; // Ambient material color
glm::vec3 Kd_material; // Diffuse material color
glm::vec3 Ks_material; // Specular material color
float Ns_material;     // Shininess exponent
...
// Object transformation states (for the main object)
bool rotateX = false;
bool rotateY = false;
bool rotateZ = false;
float objectScale = 0.5f; // Initial scale for the main object
...
// Define three point lights for 3-point lighting
PointLight keyLight;
PointLight fillLight;
PointLight backLight;

Camera camera; // Use the default constructor for Camera

// Function Prototypes
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode);
void mouse_callback(GLFWwindow* window, double xpos, double ypos);
void setupWindow(GLFWwindow*& window);
void setupShaderLightsAndMaterials(Shader& shader); // Combined setup for clarity
...
int main()
{
    ...
    // Initialize camera after shader is created
    ...
    // Initialize object position (Suzanne for example)
    glm::vec3 suzannePosition = glm::vec3(0.0f, 0.0f, 0.0f);
    float suzanneRadius = 0.5f; // Approximate radius of Suzanne model

    // Configure the three lights based on Suzanne's position and size
    configureLights(suzannePosition, suzanneRadius);

    // Initial shader setup for materials and lights
    setupShaderLightsAndMaterials(shader); // This will send initial material and light properties
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Update object rotation (if enabled)
        glm::mat4 model = glm::mat4(1);
        model = glm::translate(model, suzannePosition);
        model = glm::scale(model, glm::vec3(objectScale));

        float angle = (GLfloat)glfwGetTime(); // Continuous rotation for visual effect
        ...
        shader.setMat4("model", model); // Corrected to Shader::setMat4

        // Update light uniforms (position and whether they are enabled)
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
    ...
    if (key == GLFW_KEY_P && action == GLFW_PRESS) { rotateX = false; rotateY = false; rotateZ = false; } // Pause rotation

    // Camera movement (using Camera::setCameraPos)
...
void setupWindow(GLFWwindow*& window) {
    ...
    window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "Vivencial 2 - Iluminacao 3 Pontos", nullptr, nullptr);
    glfwMakeContextCurrent(window);

    glfwSetKeyCallback(window, key_callback);
    glfwSetCursorPosCallback(window, mouse_callback);
    glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED); // Corrected GLFW_DISABLED
...
// Configure light positions and properties for 3-point lighting
void configureLights(const glm::vec3& objectPosition, float objectRadius) {
    // Key Light (main light) - usually 45 degrees to one side and slightly above the object
    ...
    // Fill Light - opposite side of key light, lower intensity, softens shadows
    ...
    // Back Light (rim light) - behind and above the object, creates a silhouette
...
// Send light and material properties to shader
void setupShaderLightsAndMaterials(Shader& shader) {
    // Material properties (for the main object)
...
// Update light uniforms (mainly their enabled status)
void updateLightUniforms(Shader& shader) {
...
// Global vectors to store geometry data read from OBJ
...
void readFromObj(string path) {
    ...
    // Clear global vectors for new object
        ...
        if (line.length() > 0) {
            ...
            if (prefix == "v") {
                ...
                for (int i = 0; i < 3; ++i) { // Assuming triangular faces
                    ...
                    global_textures.push_back(1.0f - temp_textures[textIndex - 1].y); // Flip V coordinate for OpenGL
